        ...
        return (
          <React.Fragment key={i}>
            <div className="flex flex-col items-center gap-1 min-w-[56px]">
              ...
              <span className="text-[10px] font-medium text-center leading-tight max-w-[56px]" style={{ color: active ? "#009EDB" : done ? "#475569" : "#94a3b8" }}>{label}</span>
            </div>
            {i < STEPS.length - 1 && <div className="flex-1 h-0.5 mx-1 transition-all duration-300" style={{ background: i < current ? "#009EDB" : "#e2e8f0" }} />}
          </React.Fragment>
        );
      })}
    </div>
...
function Field({ label, required, children, hint }) {
  return (
    <div>
      ...
      {children}
      {hint && <p className="text-[11px] text-content-tertiary mt-1">{hint}</p>}
    </div>
  );
}
...
export default function RosterApplyPage() {
  ...
  // Temp states for add-item forms
  ...
  async function handleNext() {
    if (step === 0) {
      ...
      if (!memberId) {
        setLoading(true);
        try {
          const { data } = await registerRosterMember({ firstName: form.firstName, lastName: form.lastName, email: form.email, password: form.password, phone: form.phone });
          setMemberId(data.member?._id || "new");
          toast.success("Account created! Now complete your profile.");
        } catch (err) {
          toast.error(err.response?.data?.message || "Registration failed"); setLoading(false); return;
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      await updateMyRosterProfile(profileData);
        ...
        <div className="bg-white rounded-3xl p-8 shadow-sm border border-border">
          ...
          {step === 0 && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <Field label="First Name" required><input value={form.firstName} onChange={e => set("firstName", e.target.value)} className={inputCls} placeholder="John" /></Field>
                <Field label="Last Name" required><input value={form.lastName} onChange={e => set("lastName", e.target.value)} className={inputCls} placeholder="Doe" /></Field>
              </div>
              <Field label="Email Address" required><input type="email" value={form.email} onChange={e => set("email", e.target.value)} className={inputCls} placeholder="you@example.com" /></Field>
              <Field label="Password" required hint="Min 8 chars, must include uppercase, lowercase, number and special character">
                <div className="relative">
                  <input type={showPw ? "text" : "password"} value={form.password} onChange={e => set("password", e.target.value)} className={`${inputCls} pr-10`} placeholder="••••••••" />
              ...
              <div className="grid grid-cols-2 gap-4">
                <Field label="Phone Number" required><input value={form.phone} onChange={e => set("phone", e.target.value)} className={inputCls} placeholder="+1 234 567 8900" /></Field>
                <Field label="Gender">
                  <select value={form.gender} onChange={e => set("gender", e.target.value)} className={inputCls}>
                    {["Male", "Female", "Other", "Prefer not to say"].map(g => <option key={g}>{g}</option>)}
                  </select>
                </Field>
                <Field label="Nationality"><input value={form.nationality} onChange={e => set("nationality", e.target.value)} className={inputCls} placeholder="e.g. Kenyan" /></Field>
                <Field label="Country of Residence"><input value={form.countryOfResidence} onChange={e => set("countryOfResidence", e.target.value)} className={inputCls} placeholder="e.g. Kenya" /></Field>
                <Field label="City"><input value={form.city} onChange={e => set("city", e.target.value)} className={inputCls} placeholder="e.g. Nairobi" /></Field>
              </div>
            </div>
          )}
          ...
          {step === 1 && (
            <div className="space-y-4">
              ...
              <Field label="Professional Bio" hint="Tell us about your background in 2-3 sentences. Be specific about your humanitarian experience.">
                <textarea rows={4} value={form.bio} onChange={e => set("bio", e.target.value)} className={inputCls} placeholder="I am a disaster risk reduction specialist with 8 years of experience across sub-Saharan Africa..." />
              </Field>
              <Field label="Motivation Statement" hint="Why do you want to join the WDC Roster? What drives you?">
                <textarea rows={3} value={form.motivationStatement} onChange={e => set("motivationStatement", e.target.value)} className={inputCls} placeholder="My motivation to join WDC..." />
              </Field>
              <div className="grid grid-cols-2 gap-4">
                <Field label="LinkedIn Profile"><input value={form.linkedIn} onChange={e => set("linkedIn", e.target.value)} className={inputCls} placeholder="https://linkedin.com/in/..." /></Field>
                <Field label="Personal Website"><input value={form.website} onChange={e => set("website", e.target.value)} className={inputCls} placeholder="https://..." /></Field>
              </div>
          ...
          {step === 2 && (
            <div className="space-y-6">
              ...
              {/* Skills */}
              <div>
                <p className="text-sm font-semibold text-content-secondary mb-2">Skills</p>
                <div className="flex gap-2 mb-3">
                  <input value={newSkill.name} onChange={e => setNewSkill(v => ({ ...v, name: e.target.value }))} className={`${inputCls} flex-1`} placeholder="Skill name (e.g. ArcGIS, Python, HEAT)" />
                  <select value={newSkill.level} onChange={e => setNewSkill(v => ({ ...v, level: e.target.value }))} className={`${inputCls} w-36`}>
                    {SKILL_LEVELS.map(l => <option key={l}>{l}</option>)}
                  </select>
                  <button type="button" onClick={() => { if (newSkill.name.trim()) { addToArray("skills", { ...newSkill }); setNewSkill({ name: "", level: "intermediate" }); }}}
                    className="px-3 py-2 rounded-xl text-white font-bold text-sm" style={{ background: "#009EDB" }}>
                    <Plus size={16} />
                  </button>
                </div>
                <div className="flex flex-wrap gap-2">
                  {form.skills.map((s, i) => (
                    <span key={i} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold" style={{ background: "rgba(0,158,219,0.1)", color: "#009EDB" }}>
                      ...
                      <button onClick={() => removeFromArray("skills", i)}><X size={10} /></button>
                    </span>
                  ))}
                </div>
              </div>
              {/* Languages */}
              <div>
                <p className="text-sm font-semibold text-content-secondary mb-2">Languages</p>
                <div className="flex gap-2 mb-3">
                  ...
                  <button type="button" onClick={() => { if (newLang.language.trim()) { addToArray("languages", { ...newLang }); setNewLang({ language: "", proficiency: "professional" }); }}}
                    className="px-3 py-2 rounded-xl text-white font-bold" style={{ background: "#009EDB" }}>
                    <Plus size={16} />
                  </button>
                </div>
                <div className="flex flex-wrap gap-2">
                  {form.languages.map((l, i) => (
                    <span key={i} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold bg-surface-subtle border border-border text-content-secondary">
                      ...
                      <button onClick={() => removeFromArray("languages", i)}><X size={10} /></button>
                    </span>
                  ))}
                </div>
              </div>
          ...
          {step === 3 && (
            <div className="space-y-6">
              ...
              {/* Certifications */}
              <div>
                <p className="text-sm font-semibold text-content-secondary mb-2">Certifications</p>
                <div className="grid grid-cols-3 gap-2 mb-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button type="button" onClick={() => { if (newCert.name.trim()) { addToArray("certifications", { ...newCert, verified: false }); setNewCert({ name: "", issuer: "", date: "" }); }}}
              ...
              {/* Education */}
              <div>
                <p className="text-sm font-semibold text-content-secondary mb-2">Education</p>
                <div className="grid grid-cols-2 gap-2 mb-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button type="button" onClick={() => { if (newEdu.institution.trim()) { addToArray("education", { ...newEdu }); setNewEdu({ institution: "", degree: "", field: "", country: "", from: "", to: "" }); }}}
                      className="px-3 py-2 rounded-xl text-white" style={{ background: "#009EDB" }}><Plus size={16} /></button>
                  </div>
                </div>
                <div className="space-y-2">
          ...
          {step === 4 && (
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <Field label="Available From"><input type="date" value={form.availabilityDate} onChange={e => set("availabilityDate", e.target.value)} className={inputCls} /></Field>
                <Field label="Currency"><select value={form.currency} onChange={e => set("currency", e.target.value)} className={inputCls}>{["USD", "EUR", "GBP", "CHF", "KES", "XOF"].map(c => <option key={c}>{c}</option>)}</select></Field>
                <Field label="Daily Rate (approx.)" hint="Your expected daily rate for paid deployments"><input type="number" value={form.dailyRate} onChange={e => set("dailyRate", e.target.value)} className={inputCls} placeholder="200" /></Field>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2 mb-3">
                ...
                <div className="flex gap-2">
                  ...
                  <button type="button" onClick={() => { if (newRef.name.trim()) { addToArray("references", { ...newRef }); setNewRef({ name: "", organization: "", email: "", phone: "" }); }}}
                    className="px-3 py-2 rounded-xl text-white" style={{ background: "#009EDB" }}><Plus size={16} /></button>
                </div>
              </div>
              <div className="space-y-2">
          ...
          {step === 6 && (
            <div className="space-y-4">
              ...
              <div className="rounded-xl bg-surface-subtle border border-border p-4 space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-content-secondary">Name:</span><span className="font-semibold">{form.firstName} {form.lastName}</span></div>
                <div className="flex justify-between"><span className="text-content-secondary">Email:</span><span className="font-semibold">{form.email}</span></div>
                <div className="flex justify-between"><span className="text-content-secondary">Location:</span><span className="font-semibold">{form.city}, {form.countryOfResidence}</span></div>
                <div className="flex justify-between"><span className="text-content-secondary">Skills:</span><span className="font-semibold">{form.skills.length} added</span></div>
                <div className="flex justify-between"><span className="text-content-secondary">Languages:</span><span className="font-semibold">{form.languages.length} added</span></div>
                <div className="flex justify-between"><span className="text-content-secondary">Certifications:</span><span className="font-semibold">{form.certifications.length} added</span></div>
                <div className="flex justify-between"><span className="text-content-secondary">References:</span><span className="font-semibold">{form.references.length} added</span></div>
                <div className="flex justify-between"><span className="text-content-secondary">Deployment:</span><span className="font-semibold capitalize">{form.deploymentType}</span></div>
              </div>
              <label className="flex items-start gap-3 cursor-pointer">
                <input type="checkbox" required className="mt-0.5 accent-primary" />
                <span className="text-xs text-content-secondary">I confirm that all information provided is accurate. I agree to the WDC Roster terms and conditions and understand that WDC will conduct background and reference checks as part of the vetting process.</span>
              </label>
            </div>
          )}
